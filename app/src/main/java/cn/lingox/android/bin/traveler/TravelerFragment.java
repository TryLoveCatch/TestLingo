...
import cn.lingox.android.bin.traveler.view.TravelerHView;
...
public class TravelerFragment extends RecylerFragment{
    ...
    protected void loadNet() {
        addSubscription(TravelerManager.getInstatnce().loadTravelerList(mPage));
    }

    @Override
    protected void setHolderView() {
        mAdp.setHolderViews(TravelerHView.class);
    }
    ...
    public void onEvent(EventTravelerList pData){
        if(pData.data!=null){
            loadSuc(pData.data);
        }else{
            loadFail();
