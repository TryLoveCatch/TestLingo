...
public class UtilFile {
	...
	UtilFile(UtilManager pManager, Context pContext){
...
//        long totalBlocks = statfs.getBlockCount();
    ...
    private boolean isExternalStorageRemovable() {
        if (mManager.mUtilPhone.hasGingerbread()) {
	...
	private File getDirInner(String pDir){
		...
		if(UtilString.isNotBlank(tRoot)){
		    String tDir = tRoot + File.separator + pDir;
			if(createDir(tDir)){
				tFile = new File(tDir);
			}else{
				L.i(TAG, "getDir is null");
			}
		}else{
			L.i(TAG, "getDir is null");
		}
